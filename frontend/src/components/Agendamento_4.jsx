    ...
    <Card className="shadow-lg w-full flex flex-col">
      ...
      <CardContent className="p-0 flex flex-col flex-1"> 
        ...
        {calendarLoading ? (
          ...
          <div className="flex flex-1 overflow-y-auto overflow-x-hidden"> {/* Apenas scroll Y, scroll X escondido */}
            {/* Régua de Tempo */}
            ...
            <div className="flex-1 grid grid-cols-7"> {/* flex-1 para ocupar o espaço restante */}
              {semanaAtualVisivel.map((dia, indexDia) => (
                <div key={indexDia} className="border-r border-gray-300 flex flex-col"> {/* Removido min-w-[160px] */}
                  {HORARIOS_DO_DIA.map(horario => (
                    <div 
                          ...
                          idxAg === 0 && ( // Renderiza apenas o primeiro agendamento no slot
                            ...
                            >
                              ...
                              <div className="flex items-center h-full space-x-1 w-full overflow-hidden"> {/* p-0.5 removido daqui, já está no pai */}
                                ...
                                <span className="font-medium truncate flex-1 text-sm text-gray-700"> {/* Nome do paciente, text-sm */}
                                  {agendamento.patient_name}
                                </span>
                                {/* Botões de ação e outros textos (observacao, horario) removidos para um design mais limpo */}
                              </div>
                            </div>
                          )
                        ))
