...
// Gera horários de 06:00 a 23:30 em intervalos de 30 minutos
const HORARIOS_DO_DIA = (() => {
  const horarios = [];
  for (let i = 0; i < (24 - 6) * 2; i++) { 
    const totalMinutes = (6 * 60) + (i * 30);
    const hours = String(Math.floor(totalMinutes / 60)).padStart(2, '0');
    const minutes = String(totalMinutes % 60).padStart(2, '0');
    const timeString = `${hours}:${minutes}`;
    if (timeString > "23:30") break; 
  ...
  return (
    <Card className="shadow-lg w-full flex flex-col">
      <CardHeader className="bg-gradient-to-r from-blue-600 to-purple-600 text-white">
        <div className="flex justify-between items-center">
          ...
          <CardTitle className="flex items-center space-x-2 text-lg">
            <Calendar className="h-5 w-5" />
            <span>{formatarTituloSemana(semanaAtualVisivel)}</span>
          </CardTitle>
          <Button
            variant="ghost"
            size="sm"
      ...
      <CardContent className="p-0">
        {/* Cabeçalho dos dias da semana com datas */}
        <div className="grid grid-cols-7 sticky top-0 bg-gray-50 z-10 border-b">
          {semanaAtualVisivel.map((dia, index) => (
            <div key={index} className="p-2 text-center border-r">
              <div className="text-xs text-gray-500">{diasDaSemanaNomes[dia.getDay()]}</div>
        ...
        {calendarLoading ? (
          ...
          <div className="flex flex-1 overflow-auto"> {/* Container principal para a grade e régua, permite scroll se necessário */}
            {/* Régua de Tempo */}
            <div className="w-20 sticky left-0 bg-gray-50 z-20 border-r border-gray-300"> {/* Estilizada e com borda */}
              {/* Slot vazio no topo para alinhar com cabeçalho dos dias */}
              <div className="h-[60px] border-b border-gray-300"></div> {/* Altura igual ao cabeçalho do dia */}
              {HORARIOS_DO_DIA.map(horario => (
                <div key={horario} className="h-10 flex items-center justify-center text-xs text-gray-500 border-b border-gray-200"> {/* Altura fixa para cada slot de horário */}
                  {horario}
                </div>
              ))}
            </div>
            ...
            <div className="flex-1 grid grid-cols-7">
              {/* Colunas dos dias com slots de horário */}
              {semanaAtualVisivel.map((dia, indexDia) => (
                <div key={indexDia} className="border-r border-gray-300 flex flex-col"> {/* Coluna do dia */}
                  {/* Slot vazio no topo para alinhar com cabeçalho dos dias (já tratado pelo header dos dias) */}
                  {/* <div className="h-[60px] border-b border-gray-300"></div> */} {/* Não é mais necessário aqui se o header dos dias já tem essa altura */}
                  
                  {HORARIOS_DO_DIA.map(horario => (
                    <div 
                      key={`${dia.toISOString()}-${horario}`} 
                      className="h-10 border-b border-gray-200 relative" // Adicionado relative para posicionar agendamentos
                      // onClick={() => console.log(`Clicou em ${dia.toLocaleDateString()} às ${horario}`)} // Para debug futuro
                    >
                      {agendamentosPorDiaEHora[dia.toISOString().split('T')[0]] && agendamentosPorDiaEHora[dia.toISOString().split('T')[0]][horario] &&
                        agendamentosPorDiaEHora[dia.toISOString().split('T')[0]][horario].map((agendamento, idxAg) => (
                          // Por enquanto, renderiza apenas o primeiro. Lógica para múltiplos sobrepostos pode ser adicionada.
                          idxAg === 0 && (
                            <div
                              key={agendamento.id}
                              className={`absolute inset-x-0 mx-px p-1 rounded border-l-2 group cursor-pointer text-xs ${
                                agendamento.patient_is_fully_registered === false
                                  ? 'bg-red-200 text-red-900 border-red-500 hover:bg-red-300'
                                  : 'bg-purple-100 text-purple-800 border-purple-400 hover:bg-purple-200'
                              }`}
                              style={{ 
                                top: '1px', // Pequeno offset para não sobrepor totalmente a borda do slot
                                height: `calc(${agendamento.duration_minutes / 30 * 2.5}rem - 2px)`, // 2.5rem = h-10. -2px para compensar bordas/padding
                                zIndex: 10 // Para agendamentos ficarem sobre as linhas da grade
                              }}
                              onClick={() => {
                            ...
                            >
                              <div className="flex justify-between items-start h-full">
                                <div className="flex-1 min-w-0 flex flex-col justify-between h-full">
                                  <div>
                                    ...
                                    {agendamento.observacao && (
                                      <div className="text-gray-500 mt-0.5 truncate" title={agendamento.observacao}>
                                        {agendamento.observacao}
                                      </div>
                                    )}
                                  </div>
                                  <div className="text-purple-700 mt-0.5 whitespace-nowrap text-[10px]">
                                ...
                                <div className="flex flex-col items-center opacity-0 group-hover:opacity-100 transition-opacity space-y-0.5">
                                  <Button variant="ghost" size="icon" className="h-5 w-5 p-0 hover:bg-blue-100" title="Editar agendamento"
                                    onClick={(e) => { e.stopPropagation(); navigate(`/agendamentos/editar/${agendamento.id}`); }}>
                                    <Edit className="h-3 w-3 text-blue-600" />
                                  </Button>
                                  <Button variant="ghost" size="icon" className="h-5 w-5 p-0 hover:bg-red-100" title="Excluir agendamento"
                                    onClick={(e) => { e.stopPropagation(); excluirAgendamento(agendamento.id); }}>
